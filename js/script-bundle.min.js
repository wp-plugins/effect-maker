...
function AppManagement(clientContext, webPartMode) {
    ...
    AppManagement.prototype.init = function (divIDinit, divIDmain) {
        ...
        if (EffectMaker.getEnvironment() == EffectMaker.SHAREPOINT) {
            ...
            LM.checkLists(manageList, function () { UCM.initWorkFolder(webPartMode); });
        }
        else if (EffectMaker.getEnvironment() == EffectMaker.WORDPRESS) {
            UCM.initWorkFolder(webPartMode);
        }
...
function Configurator(configurationHTMLID) {
    ...
    Configurator.prototype.moveListItem = function (name, up) {
        var n = $.inArray(name, listTracker);
        if (up && n > 0) {
 //           var a = $(listTracker[n]).html();
 //           $(listTracker[n]).remove();
...
//            $(listTracker[n]).remove();
    ...
    Configurator.prototype.applyDialog = function()
    {
        defaultValues = '';
        containsarrays = '';
        var output = '';
        for (var i = 0; i < configuratorItems.length; i++) {
            ...
            if (item.type == this.FILEFIELD) {
                var value = $('#' + item.name + ind).val();
                if (value == undefined) value = '';
                defaultValues += item.name + ': "' + value + '", ';
                if (value.indexOf('em_') > -1) output += instanceName + '_' + item.name + ' = "' + defaultImageFolder + value + '"' + ';';
                else output += instanceName + '_' + item.name + ' = "' + userImageFolder + value + '"' + ';';
            }
            if (item.type == this.CHECKBOX) {
                ...
                for (var k = 0; k < listTracker.length; k++) {
                    ...
                    for (var j = 0; j < item.param1.length; j++) {
                        ...
                        if (item2.type == this.FILEFIELD) {
                            ...
                            if (value.indexOf('em_') > -1) outputs[j] += instanceName + '_' + item2.name + '[' + k + ']  = "' + defaultImageFolder + value + '"' + ';';
                            else outputs[j] += instanceName + '_' + item2.name + '[' + k + ']  = "' + userImageFolder + value + '"' + ';';
                        }
                    }
                }
                for (var j = 0; j < outputs.length; j++)
    ...
    Configurator.prototype.getCodeFromValues = function (values) {
        ...
        for (var i = 0; i < configuratorItems.length; i++) {
            ...
            if (item.type == this.FILEFIELD) {
                if (values[item.name].indexOf('em_')>-1) output += item.name + ' = "' + defaultImageFolder + values[item.name] + '"' + ';';
                else output += item.name + ' = "' + userImageFolder + values[item.name] + '"' + ';';
            }
            if (item.type == this.CHECKBOX) {
                output += instanceName + '_' + item.name + ' = ' + values[item.name] + ';';
            }
...
function EffectMakerConfiguration() {
    this.SHAREPOINT = 0, this.WORDPRESS = 1, this.DRUPAL = 2, this.JOOMBLA = 3;
    var hostUrl = null;
...
function EffectManagement(clientContext) {
    ...
    EffectManagement.prototype.setupGallery = function (galleryHTMLID, galleryHTMLID2, userConfigurations) {
        ...
        categorycontent['Your configurations'] = '<div id="efcat-' + (i + 1) + '">';
        ...
        for (var name in effects) {
            ...
            content = '';
            content += '<div style="float:left; width:150px; cursor:pointer; padding-left:10px;" onClick="AM.effectSelected('+singlequote + item.name + singlequote+');">';
  //          categorycontent[item.category] += '<div style="width:150px">';
...
function ParameterManagement(clientContext) {
    ...
    ParameterManagement.prototype.setParameter = function (Name, Value) {
        ...
        if (Name in ParameterValues) {
            ...
            this.updateParameter(ParameterIDs[Name], Value);
        }
        else {
            ParameterValues[Name] = Value;
    ...
    ParameterManagement.prototype.updateParameter = function (id, Value) {
        if (EffectMaker.getEnvironment() == EffectMaker.SHAREPOINT) {
            var oList = clientContext.get_web().get_lists().getByTitle('Parameters');
...
function UserContentManagement(clientContext, hostWebContext) {
    ...
    var defaultImages = ['em_batteries.png', 'em_bridge.png', 'em_church.png', 'em_church2.png', 'em_church3.png',
        ...
        'em_statue2.png', 'em_tree.png', 'em_white_rose.png', 'em_vina.png', 'em_vina2.png', 'em_windmill.png', 'em_woods.png'];
    ...
    UserContentManagement.prototype.initWorkFolder = function () {
        if (EffectMaker.getEnvironment() == EffectMaker.SHAREPOINT) {
            ...
            clientContext.load(this.collListItem);
            clientContext.executeQueryAsync(
                Function.createDelegate(this, this.onQuerySucceeded_initWorkFolder),
                Function.createDelegate(this, this.onQueryFailed_initWorkFolder)
            );
        }
        else if (EffectMaker.getEnvironment() == EffectMaker.WORDPRESS) {
            ...
            $.get(ajaxurl,ajaxAction, function (data) {
                ...
                var i = 0;
                $($xml).find('item').each(function () {
                    fileNames[i] = $(this).find('name').text();
                    i++;
                });
    ...
    UserContentManagement.prototype.deleteUserConfiguration = function (configurationname) {
        if (EffectMaker.getEnvironment() == EffectMaker.SHAREPOINT) {
            ...
            clientContext.executeQueryAsync(function () {
                EM.addStatusMessage('Deleted configuration with ID: '+configuration);
            },
               function (sender, args) {
